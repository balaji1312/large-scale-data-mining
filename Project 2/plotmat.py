...
def plot_mat(mat, xticklabels = None, yticklabels = None, pic_fname = None, size=(-1,-1), if_show_values = True, textsize=15, padding = 0.1,
    ...
    if colorbar:
        plt.colorbar(im,fraction=0.046, pad=padding)
    ...
    if if_show_values:
        show_values(im, fontsize=textsize)
    ...
    plt.tight_layout()
    if pic_fname:
        plt.savefig(pic_fname, dpi=500, bbox_inches='tight', transparent=True)
    plt.show()
    plt.close()